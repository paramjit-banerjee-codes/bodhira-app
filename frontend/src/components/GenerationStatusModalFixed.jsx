...
export default function GenerationStatusModal({
    ...
    const animateProgress = () => {
      ...
      if (isBackendComplete) {
        ...
        setDisplayProgress(prev => {
          ...
          const newProgress = prev + diff * 0.2; // Smooth lerp
          return Math.min(newProgress, 100);
        });

        // Check if we've reached 100%
        if (targetProgress >= 100 && displayProgress >= 99.5) {
      ...
      // Backend still working - normal progress
        ...
        setDisplayProgress(prev => {
          const diff = targetProgress - prev;
          const newProgress = prev + diff * 0.1; // Smooth lerp
  ...
  }, [status, isOpen, isBackendComplete, backendResult, displayProgress]);
